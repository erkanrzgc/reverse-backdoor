...
def search_files(protocol, pattern='*.docx;*.xlsx;*.pdf;*.txt', root_path='/'):
    ...
    try:
        if os.name == 'nt':
            ...
            for ext in exts:
                ...
                results.extend(r.stdout.decode('utf-8', errors='replace').strip().split('\n'))
        else:
...
def search_content(protocol, keyword, root_path='/'):
    try:
        if os.name == 'nt':
            r = subprocess.run(

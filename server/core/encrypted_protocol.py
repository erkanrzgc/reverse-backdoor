...
class EncryptedProtocol:
    """Encrypted JSON wire protocol with ECDH key exchange + AES-256-GCM (server-side)."""
    ...
    def perform_key_exchange(self) -> None:
        client_pub = self._sock.recv(1024)
        if not client_pub:
            raise ConnectionError("Connection closed during key exchange")

        server_pub = self._crypto.public_key_bytes
        self._sock.sendall(server_pub)

        self._crypto.compute_shared_key(client_pub)
